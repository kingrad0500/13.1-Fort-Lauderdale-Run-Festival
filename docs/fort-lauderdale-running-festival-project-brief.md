...
| 29.19 | §17, §7.2, §25 — **reverses 29.13** | The top event-facts strip is now a continuous marquee on all screen sizes, with a pause control | Client direction. 29.13 kept it stationary to honour §7.2 and §25, but the stationary version was *not delivering* the information those rules protect: the row overflowed below ~1000px with its scrollbar hidden, so on a phone the tail — including the price-increase deadline — was invisible with no affordance suggesting it existed. Mitigations: a real pause button (44px, `aria-pressed`), pause on hover and focus, a static scrollable row for reduced-motion/no-JS, and the duplicate loop copy `aria-hidden` so facts are announced once. Every fact on the bar also appears stationary elsewhere on the site, so nothing lives only inside moving text |
| 29.20 | §16 | The top bar is now 44px tall (was ~36px) | The pause control §29.19 requires is a real target, and the project holds interactive controls to 44px. The bar sits above the header, so `--header-h` and the hero overlap are unaffected |
